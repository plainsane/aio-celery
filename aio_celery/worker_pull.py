    ...
    *,
...
) -> tuple[Task, AnnotatedTask] | None:
    """Prepare a task for execution from an incoming message.
    
    Returns None if the task should be rejected (unregistered).
    """
    with contextlib.ExitStack() as stack:
        ...
        running_task = RunningTask(
            asyncio_task=None,  # Will be set when asyncio.Task is created
            task_id=task_id,
            task_name=task_name,
            received=_iso_now(),
            state="SLEEPING",
...
async def execute_task_async(
    ...
    """Execute a task asynchronously. This runs independently after message ACK."""
    app = task.app
    args = (task, *task.request.args) if annotated_task.bind else task.request.args
    logger.debug(
        "Task %s[%s] started executing",
        task.name,
        task.request.id,
    )
    timestamp_start = time.monotonic()
    
    # Update the running task to point to the current asyncio task
    if task.request.id in _STATE.running_tasks:
        _STATE.running_tasks[task.request.id].asyncio_task = asyncio.current_task()
    ...
    try:
        ...
        # Since we've already ACKed the message, we can't reject it
        # We must log the failure and update task state
        ...
        with contextlib.suppress(KeyError):
            del _STATE.running_tasks[task.request.id]
...
async def run(args: argparse.Namespace) -> None:
    ...
    async with app.setup():
        ...
        async with server, connection, connection.channel() as channel:
            ...
            logger.info("Starting pull-based worker. Waiting for messages. To exit press CTRL+C")
            
            # Main polling loop
            running_tasks: list[asyncio.Task] = []
            max_concurrent_tasks = args.concurrency
            poll_timeout = app.conf.worker_poll_timeout
                ...
                while True:
                    ...
                    for task in completed_tasks:
                        ...
                        if task.exception() is not None:
                            logger.error("Background task failed with exception", exc_info=task.exception())
                    ...
                    if len(running_tasks) < max_concurrent_tasks:
                        ...
                        for queue in queues:
                            try:
                                message = await queue.get(timeout=poll_timeout, fail=False)
                                if message is not None:
                                    # Prepare the task (preflight checks)
                                    task_data = await prepare_task(
                                        message, 
                                        app=app, 
                                        gc_is_paused=gc_is_paused
                                    )
                                    ...
                                    if task_data is not None:
                                        task, annotated_task = task_data
                                        
                                        # Create asyncio.Task (starts executing immediately)
                                        asyncio_task = asyncio.create_task(
                                            execute_task_async(task, annotated_task)
                                        )
                                        
                                        # ACK the message NOW (before task completes)
                                        await message.ack()
                                        
                                        # Add to our tracking list
                                        running_tasks.append(asyncio_task)
                                        message_received = True
                                        break  # Got a message, don't check other queues
                            except asyncio.TimeoutError:
                                # No message available from this queue
                                continue
                        
                        if not message_received:
                            # No messages available from any queue, wait a bit
                            await asyncio.sleep(0.1)
                    else:
                        # No room for more tasks, wait before checking again
                        await asyncio.sleep(0.1)
            except KeyboardInterrupt:
                logger.info("Received shutdown signal, waiting for running tasks to complete...")
                if running_tasks:
